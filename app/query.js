...
const getContacts = (request, response) => {
  pool.query('SELECT * FROM contactsdb', (error, results) => {
    if(error) {
      throw error
    }
...
const getContactById = (request, response) => {
  ...
  pool.query('SELECT * FROM contactsdb WHERE _id = $1', [id], (error, results) => {
    if(error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const addContact = (request, response) => {
  ...
  pool.query('INSERT INTO contactsdb (first_name, last_name, address, email_address, contact_number) VALUES ($1, $2, $3, $4, $5)', 
...
const updateContact = (request, response) => {
  ...
  pool.query(
    'UPDATE contactsdb SET first_name = $1, last_name = $2, address = $3, email_address = $4, contact_number = $5 WHERE _id = $6',
...
const deleteContact = (request, response) => {
  ...
  pool.query('DELETE FROM contactsdb WHERE _id = $1', [id], (error, results) => {
    if(error){
      throw error
    }
